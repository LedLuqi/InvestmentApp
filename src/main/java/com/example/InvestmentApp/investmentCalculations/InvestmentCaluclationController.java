...
public class InvestmentCaluclationController {
    ...
    public @ResponseBody InvestmentCalculationInfo getInvestmentAndHistoryOfCalculation (@PathVariable("id") Long id) {
        return calculationService.getInfoAndHistoryCalculations(id);
    }

    @PostMapping("/{id}/calculations")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody AddedInvestmentCalculation caluclate(@RequestBody InvestmentCalculationDTO investmentCalculationDTO, @PathVariable("id") Long id) throws FutureInvestmentCalculationException {
